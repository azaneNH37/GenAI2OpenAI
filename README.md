...
**关键设计**：`provider/genai.py` 是唯一与上游通信的位置，将 Token 续期、SSE 流式、重试三件事收口在一处；工具翻译层通过 prompt 注入 + 输出解析，让不支持原生 Function Calling 的上游模型也能在 OpenAI 生态工具中工作。
